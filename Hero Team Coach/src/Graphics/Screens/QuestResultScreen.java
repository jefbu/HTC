...
public class QuestResultScreen extends JPanel {
	...
	public QuestResultScreen() {
		...
		textAreaPanel.setPreferredSize(new Dimension(GameScreen.WIDTH / 2, GameScreen.HEIGHT / 2));
	...
	public void fillScreen() {
		...
		for (int i = 0; i < GameScreen.questers.size(); i++) {
			...
			if (GameScreen.questers.get(i).skill > heroInt[i * 5 + 2]) {
				output = output + GameScreen.questers.get(i).name
						+ " climbed onto the next rung of the skill ladder \n";
			}
			...
			if (GameScreen.questers.get(i).personality > heroInt[i * 5 + 4]) {
				output = output + GameScreen.questers.get(i).name
						+ " leaves all onlookers more awestruck than before \n";
			}

		}

		GameScreen.runner = 0;
		textResultArea.setText(output);
